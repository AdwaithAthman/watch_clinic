...
module.exports = {
  ...
  validateCoupon: (data, userId, totalAmount) => {
    console.log("Enterd Validate Coupen in helpers");
    return new Promise(async (resolve, reject) => {
      try {
        obj = {};
        let date = new Date();
        ...
        if (coupon) {
          ...
          if (userCheck) {
            ...
            if (date <= coupon.endDateIso) {
              ...
              let discountValue = parseInt(
                ((total * percentage) / 100).toFixed()
              );
              ...
              resolve(obj);
              console.log("Price after Discount", obj.total);
